...
初版把所有內容堆在單一直欄，「變更」分頁必須一路往下捲才看得到提交框，
而右側的差異區被擠得很窄，橫向空間浪費。

改為固定框架：工具列、repository 標頭與分頁列固定，只有內容區捲動。
「變更」分頁再切成雙欄 —— 左邊是檔案清單與提交框（提交框固定在底部，
不會被捲走），右邊是差異，佔滿剩餘的高度與寬度。分支管理從三排堆疊的
控制項改為獨立分頁，每條分支一列，操作就在該列上。

還原點原本埋在同步狀態的按鈕列裡，只看得到最近一個；移到分支分頁後
可以看到全部並選擇還原到哪一個。
